...
const enableMfaJustSubmitted = ref(false)
...
function handleToggleMfa() {
  ...
  errorMessage.value = ''
  if (mfaEnabled.value) {
    // MFA is ON -> user clicked to turn OFF -> show disable modal
    ...
    // MFA is OFF -> user clicked to turn ON -> show enable modal
    enableMfaJustSubmitted.value = false
...
async function submitEnableMfa() {
  ...
  try {
    ...
    const data = response.data
    enableMfaJustSubmitted.value = true
...
function onEnableMfaModalClose() {
  if (!enableMfaJustSubmitted.value) {
    mfaEnabled.value = false
  }
  enableMfaJustSubmitted.value = false
}
